...
class Search {
  ...
  //LIVE SEARCH
  typingLogic() {
    //timer will only run if the value is different to the previous search value
    if (this.searchField.value != this.previousValue) {
      //restarts the timer every time the method is called
      clearTimeout(this.typingTimer);

      //if the searchField is not blank
  ...
  //KEY PRESS OPEN / CLOSE
  keyPressDispatcher(e) {
    //prevents unwanted behaviours when user is focussed on another input / textarea
    if (
      e.keyCode == 83 &&
      !this.isOverlayOpen &&
